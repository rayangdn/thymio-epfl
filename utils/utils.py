...
def record_frames(original_frame=None, processed_frame=None, trajectory_frame=None, video_writer=None, output_path='output.mp4', DT=0.1):
    ...
    if video_writer is None and combined_frame is not None:
        # Initialize video writer with frame properties
        height, width = combined_frame.shape[:2]
        video_writer = cv2.VideoWriter(
            output_path,
            cv2.VideoWriter_fourcc(*'mp4v'),
            1/DT,  # FPS based on your DT
            (width, height)
        )
    
    if combined_frame is not None and video_writer is not None:
